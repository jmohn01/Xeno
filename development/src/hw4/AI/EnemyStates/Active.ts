...
export default class Active extends EnemyState {
    ...
    pickRetreatPath(pathToPlayer: NavigationPath){
        const retreatDis = 100;
        const toPosition = this.owner.position.clone().sub(pathToPlayer.getMoveDirection(this.owner).scaled(retreatDis));
        this.parent.retreatPath = this.owner.getScene().getNavigationManager().getPath(hw4_Names.NAVMESH, this.owner.position, toPosition, true); 
        // console.log(`direction: ${pathToPlayer.getMoveDirection(this.owner)}, next pos: ${toPosition}, newPath: ${this.parent.retreatPath}`);
    }
