...
import { effectsData } from "./Attack/internal";
...
export default class BattleManager {
    ...
    handlePointAtk(to: BattlerAI, dmg: number, effects: effectsData) {
        to.damage(dmg);
        BattleManager.addEffects(effects, to);
    }

    handleAOEAtk(from: Vec2, r: number, dmg: number, effects: effectsData, atkerType: XENO_ATKER_TYPE) {
        const r2 = r * r;
        switch (atkerType) {
    ...
    private static addEffects(data: effectsData, target: BattlerAI) {
        if (data.fire) {
